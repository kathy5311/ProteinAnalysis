...
def H_func(ss_dict,atom_dict,phi_dict,psi_dict):
    Hdict=Hdict_func(atom_dict)
    for L in Hdict:
        if L[0]+4==L[1]:
...
def E_angle(ss_dict,atom_dict):
    phi_dict=phi_dict_func(atom_dict)
    psi_dict=psi_dict_func(atom_dict)
    for i in atom_dict:
...
def process_pdb_file(file_path):
    ...
    for ch in atom_dict:
        ...
        b=H_func(ss_dict[ch],Hdict_func(atom_dict)[ch],phi_dict_func(atom_dict)[ch],psi_dict_func(atom_dict)[ch])
        ss_dict[ch]=b
        c=E_angle(ss_dict[ch],atom_dict[ch],phi_dict_func(atom_dict)[ch],psi_dict_func(atom_dict)[ch])
        ss_dict[ch]=c
        d=E_func(ss_dict[ch],Hdict_func(atom_dict)[ch])
        ss_dict[ch]=d
